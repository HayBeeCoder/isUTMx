...
void fifth_page_ui(U8G2_ST7920_128X64_F_SW_SPI u8g2, int page, int page_value_address, char key, String test, int address, String &inputt_value, String &sensor_rating_in_kg, String &targetForce, int selected_page)
{
    ...
    if (key)
    {
        ...
        else if (key == '#')
        {
            ...
            double TOLERANCE = 0.01 * SENSOR_RATING_IN_N;
            EEPROM.commit();
            ...
            if (
                ...
                (page == 51 &&
                 ((SENSOR_RATING_IN_N - inputt_value.toFloat()) >= TOLERANCE)))
            {
                if (page == 5)
                {
